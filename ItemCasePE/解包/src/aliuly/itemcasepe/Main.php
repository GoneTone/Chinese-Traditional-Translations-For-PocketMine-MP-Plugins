...
class Main extends PluginBase implements CommandExecutor,Listener {
	...
	private function access(CommandSender $sender, $permission) {
		...
		$sender->sendMessage("您沒有權限這樣做。\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
		return false;
	}
	private function inGame(CommandSender $sender,$msg = true) {
		if ($sender instanceof Player) return true;
		if ($msg) $sender->sendMessage("您只可以在遊戲中使用此指令　§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
		return false;
	}
	// Standard call-backs
	...
	private function cmdCancelAdd(CommandSender $c) {
		...
		if (!isset($this->touches[$c->getName()])) {
			$c->sendMessage("不加入 ItemCase\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
			return true;
		}
		unset($this->touches[$c->getName()]);
		$c->sendMessage("添加 ItemCase 已取消\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
		return true;
	}
	private function cmdAdd(CommandSender $c) {
		if (!$this->inGame($c)) return true;
		$c->sendMessage("拿著一個項目，點擊目標方塊\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
	...
	public function onPlayerInteract(PlayerInteractEvent $ev){
		...
		if ($this->classic) {
			if ($bl->getID() != Block::GLASS) {
				if ($bl->getID() == Block::SLAB)
					...
					$pl->sendMessage("你必須把 item cases 放置於地磚");
					$pl->sendMessage("或玻璃磚！\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
					return;
				}
			}
		...
		if (!$this->addItemCase($bl->getLevel(),$cid,
										...
										$item->getCount())) {
			$pl->sendMessage("已經有一個 ItemCase 在那裡！\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
		} else {
			$pl->sendMessage("ItemCase 放置！\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
		}
	...
	public function onBlockBreak(BlockBreakEvent $ev){
		...
		if (isset($this->cases[$lv->getName()][$cid])) {
			...
			$pl->sendMessage("銷毀 ItemCase $cid　\n§7翻譯：PocketMine-MP 伺服插件資源網 (http://PocketMinePlugins.reh.tw)");
