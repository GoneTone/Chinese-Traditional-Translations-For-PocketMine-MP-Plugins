...
class DevTools extends PluginBase implements CommandExecutor{
	...
	private function permissionCheckCommand(CommandSender $sender, Command $command, $label, array $args){
		...
		if($target !== $sender and !$sender->hasPermission("devtools.command.checkperm.other")){
			$sender->sendMessage(TextFormat::RED . "您沒有權限查看其他玩家。\n§7翻譯：旋風之音 (http://PocketMinePlugins.reh.tw)");
			return true;
		}else{
			$sender->sendMessage(TextFormat::GREEN . "---- ".TextFormat::WHITE . "權限節點".$node.TextFormat::GREEN. " ----");
			$perm = $this->getServer()->getPluginManager()->getPermission($node);
			if($perm instanceof Permission){
				$desc = TextFormat::GOLD . "描述: ".TextFormat::WHITE . $perm->getDescription()."\n";
				...
				$desc = TextFormat::RED . "許可不存在\n";
				$desc .= TextFormat::GOLD . "Default: ".TextFormat::WHITE . Permission::$DEFAULT_PERMISSION."\n";
			}
			$sender->sendMessage($desc);
			$sender->sendMessage(TextFormat::YELLOW . $target->getName() . TextFormat::WHITE . " 已經將其設置為 ".($target->hasPermission($node) === true ? TextFormat::GREEN . "true" : TextFormat::RED . "false"));
			return true;
		}
	}
	...
	private function makePluginLoader(CommandSender $sender, Command $command, $label, array $args){
		...
		if(file_exists($pharPath)){
			$sender->sendMessage("Phar插件已經存在，覆蓋...");
			@unlink($pharPath);
		}
		...
		$phar->stopBuffering();
		$sender->sendMessage("文件夾插件加載已創建 ".$pharPath);
		return true;
	}
	...
	private function makePluginCommand(CommandSender $sender, Command $command, $label, array $args){
		...
		if($pluginName === "" or !(($plugin = Server::getInstance()->getPluginManager()->getPlugin($pluginName)) instanceof Plugin)){
			$sender->sendMessage(TextFormat::RED . "無效的插件名稱，檢查該名稱個案。\n§7翻譯：旋風之音 (http://PocketMinePlugins.reh.tw)");
			return true;
		}
		$description = $plugin->getDescription();

		if(!($plugin->getPluginLoader() instanceof FolderPluginLoader)){
			$sender->sendMessage(TextFormat::RED . "插件 ".$description->getName()." 不是在文件夾結構。\n§7翻譯：旋風之音 (http://PocketMinePlugins.reh.tw)");
			return true;
		}
		...
		$phar->stopBuffering();
		$sender->sendMessage("Phar插件 ".$description->getName() ." v".$description->getVersion()." 已創建 ".$pharPath);
		return true;
	}
